...
開発者ごとに異なる開発環境で作業してしまう可能性が生じる。Q4の回答を参照。
...
➀開発環境統一の観点。例えばQ4の例を再度考えると、同じnumpy2.0であってもWindows環境とLinux環境で内容が微妙に異なることがある。使うべきnumpy2.0がLinux版であるならば、「Linux版numpy2.0をハッシュ関数に入力して得られたハッシュ値」がpoetry.lockのnumpy2.0の項目に記載・記録されている。　　　　ここで、別の開発者が、誤ってLinux環境ではなくWindows環境において`poetry install`してしまったとする。すると、Windows版numpy2.0がインストールされるため、先ほどとは異なるハッシュ値が出力され、poetry.lockのハッシュ値が更新される。開発者は、ハッシュ値が更新されていることから、「自身の開発環境が誤っているかも知れない」と気付くことができる。
